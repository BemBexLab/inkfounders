      ...
      >
        <div className="w-full">
          ...
          >
            ...
            <motion.h2
              className="plus-jakarta uppercase mb-4 text-2xl font-medium text-black sm:text-4xl lg:mb-5 lg:text-5xl [word-spacing:4px]"
              variants={fadeUpVariants}
            >
              DIY Publishing vs. 
            ...
            <motion.p
              className="plus-jakarta mx-auto max-w-7xl text-sm leading-relaxed text-black sm:text-base lg:text-lg font-medium [word-spacing:2px]"
              style={{ marginBottom: "clamp(2rem, 6vw, 5rem)" }}
              variants={fadeUpVariants}
            >
              We are not here to do what everyone else does. Here's an honest
              look at what separates us from going it alone or settling for
              less.
            </motion.p>
          </motion.div>

          <motion.div
            className="plus-jakarta relative"
          ...
          >
            ...
            {/* Black Image Col */}
            <motion.div
              className="plus-jakarta absolute right-0 flex flex-col overflow-hidden rounded-xl lg:rounded-2xl"
              style={{
            ...
            >
              <motion.div
                ...
                style={{
                  height: `${DARK_OVERFLOW + HEADER_HEIGHT}px`,
                  flexShrink: 0,
                }}
              ...
              {rows.map((row, i) => (
                ...
                >
                  ...
                  />
                  <div
                    className="plus-jakarta flex items-center justify-center px-2 text-center sm:px-4"
                    style={{ height: `${ROW_HEIGHT}px` }}
                  >
                    {row.ink.yellow ? (
                      <span
                        className="plus-jakarta font-semibold leading-tight"
                        style={{
                          color: "#DADD39",
                          fontSize: "clamp(0.55rem, 1.4vw, 0.98rem)",
                        }}
                      >
                        {row.ink.value}
                      </span>
                      ...
                      <span
                        style={{
                          fontSize: "clamp(0.55rem, 1.4vw, 0.99rem)",
                          lineHeight: 1.3,
                        }}
                      >
                        <span style={{ color: "#C8D400", fontWeight: 400 }}>
                          {CHECK_MARK}{" "}
                        </span>
                        <span className="plus-jakarta font-bold text-[#DADD39]">
                          {row.ink.value}
                        </span>
                      </span>
                    )}
            ...
            <motion.div className="plus-jakarta flex flex-col" variants={containerVariants}>

              {/* Table Head */}
              <motion.div
                className="plus-jakarta grid overflow-hidden rounded-l-xl"
                style={{
              ...
              {rows.map((row, i) => (
                <motion.div
                  ...
                  whileHover={{
                    x: 4,
                    backgroundColor: row.shaded ? "#efefef" : "#fafafa",
                  }}
                  transition={{ delay: i * 0.03 }}
                >
                  {/* First Row */}
                  <div className="plus-jakarta flex items-center px-3 sm:px-5 lg:px-7">
                    <span
                      style={{
                  ...
                  {/* Third Row */}
                  <div
                    className="plus-jakarta flex items-center justify-center px-1 text-center sm:px-3"
                    style={{ borderLeft: "1px solid #e5e5e5" }}
                  >
